...
export class CreateAppointmentComponent implements OnInit {
 ...
 GetSitebySpecialityId(SpecialtyId: any) {
  ...
  res.result.forEach((item: any) => {
    unique.set(item.id, item); // ya item.code, jo bhi unique ho
  });
  this.specialities = Array.from(unique.values());
});

      let provider=this.selectedProvider||0
      let facility=this.selectedFacility||0
      let site=this.selectedSite||0
