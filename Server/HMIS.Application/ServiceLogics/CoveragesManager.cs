...
namespace HMIS.Service.ServiceLogics
{
    public class CoveragesManager : ICoveragesManager
    {
        ...
        //}
        public CoveragesManager(HIMSDBContext context, IConfiguration configuration)
        {
            _context = context;
            Configuration = configuration;
        }
            ...
            try
            {
                ...
                if (regInsert.regInsurancePolicy != null && regInsert.regInsurancePolicy.Count > 0)
                {
                    foreach (var item in regInsert.regInsurancePolicy)
                    {
                        ...
                        Data.Models.InsuredPolicy ip = new Data.Models.InsuredPolicy()
                        {
                            IsDeleted = false,
                ...
                if (regInsert.regDeduct != null && regInsert.regDeduct.Count > 0)
                {
                    ...
                    {
                        Data.Models.DeductiblePercent dp = new Data.Models.DeductiblePercent()
                        {
                            IsDeleted = false,
                            SubscriberId = item.SubscriberId,
